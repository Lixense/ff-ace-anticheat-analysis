...
teamproxy/
  README_FINDINGS.md                 full teardown of a real "panel" (proxy TEAM), every layer
  README.md                          folder intro + what is still locked
  analysis_scripts/                  the decrypt scripts used to break its string layers
  evidence/                          decoded strings, hmac method source, native key dumps
  payload_modes/                     the 4 "modes" it injects (hybridclr esp patches + config)
  ghidra_out/                        ghidra decompile of its native lib123.so
  xor_key.txt                        the 6978 char static xor key recovered from the app
```
...
## Exposed: a real "panel" taken apart (proxy TEAM)

People keep asking me "so what about the panels that do work?" so i went and bought one,
took it apart completely and put the whole thing in `teamproxy/`. This is what these
"ff mod panels" actually are. The full writeup with every file, every decrypted string,
the server urls and the payloads is `teamproxy/README_FINDINGS.md`. The raw evidence
(apk, runtime data pull, ghidra decompile of the native lib, decrypt scripts, the
recovered xor key) is all in the `teamproxy/` folder.

the one i took apart is "SX2 TEAM PROXY" (com.Sx2.Team.Ban):
- package com.Sx2.Team.Ban, app name "SX2 TEAM PROXY", license server sx2lador.online,
  telegram t.me/sx2team/3822, license keys look like SX2-TEAM-XXXXXXXX
- his app needs shizuku (root shell) and his api ties every install to your device id
  plus his hmac secret (Sx2TeamAppSecureKey2026@!)
...
### why it is esp only and never a real aimbot
...
fire/shoot/trigger control, no target lock, no smoothing, no silent aim. the one
set_localRotation in the patch is just the overlay canvas rotating itself, not your gun.
...
the app was protected like this:
